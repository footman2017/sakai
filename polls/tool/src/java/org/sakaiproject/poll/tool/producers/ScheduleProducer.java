...
@Slf4j
public class ScheduleProducer implements ViewComponentProducer,
DefaultView,NavigationCaseReporter {
	public static final String VIEW_ID = "scheduleView";
	...
	public void fillComponents(UIContainer tofill, ViewParameters viewparams,
		...
		UIOutput.make(tofill, "schedule-list-title", messageLocator.getMessage("schedule_list_title"));	
		...
		if (this.isAllowedPollAdd() || this.isSiteOwner() ) {
			...
			if (this.isAllowedPollAdd()) {
				...
				UIInternalLink.make(actions,"action-overview", UIMessage.make("action_overview"),new SimpleViewParameters(PermissionsProducer.VIEW_ID));
                                UIInternalLink.make(actions,"action-overview",UIMessage.make("action_add_poll"),
						new PollViewParameters(AddPollProducer.VIEW_ID, "New 0"));
                                UIInternalLink.make(actions,"action-membership", UIMessage.make("action_membership"),new SimpleViewParameters(PermissionsProducer.VIEW_ID));
                                UIInternalLink.make(actions,"action-options", UIMessage.make("action_options"),new SimpleViewParameters(PermissionsProducer.VIEW_ID));
                                UIInternalLink.make(actions,"action-schedule", UIMessage.make("action_schedule"),new SimpleViewParameters(PermissionsProducer.VIEW_ID));
			}       
			if (this.isSiteOwner()) {
...
//		/*UIMessage.make(deleteForm, "poll-question-title","poll_question_title");
		UIMessage.make(tofill, "schedule-no-title", "schedule_no_title");
		UIMessage.make(tofill, "schedule-time-title", "schedule_time_title");
		UIMessage.make(tofill, "schedule-mk-title", "schedule_mk_title");
		UIMessage.make(tofill, "schedule-tepr-title", "schedule_tepr_title");
                UIMessage.make(tofill, "schedule-dosen-title", "schedule_dosen_title");
                UIMessage.make(tofill, "schedule-ruang-title", "schedule_ruang_title");
//		
